...
<?php include_once "package_reportrptinfo.php" ?>
<?php
...
$package_report_rpt = NULL; // Initialize page object first

class crpackage_report_rpt extends crpackage_report {
	...
	// Page object name
	var $PageObjName = 'package_report_rpt';
	...
	function __construct() {
		...
		// Table object (package_report)
		if (!isset($GLOBALS["package_report"])) {
			$GLOBALS["package_report"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["package_report"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'package report', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fpackage_reportrpt";
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewrExportLink ewEmail\" title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_package_report\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_package_report',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fpackage_reportrpt\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fpackage_reportrpt\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fpackage_reportrpt\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = FALSE;
	...
	function GetRow($opt) {
		...
		if ($opt == 1) { // Get first row
				...
				$this->FirstRowData['peroid'] = ewr_Conv($rs->fields('peroid'), 200);
				$this->FirstRowData['packagename'] = ewr_Conv($rs->fields('packagename'), 200);
				$this->FirstRowData['price'] = ewr_Conv($rs->fields('price'), 3);
				$this->FirstRowData['packagedetails'] = ewr_Conv($rs->fields('packagedetails'), 200);
				...
				$this->FirstRowData['pdate'] = ewr_Conv($rs->fields('pdate'), 133);
		} else { // Get next row
			$rs->MoveNext();
		}
		if (!$rs->EOF) {
			$this->peroid->setDbValue($rs->fields('peroid'));
			$this->packagename->setDbValue($rs->fields('packagename'));
			$this->price->setDbValue($rs->fields('price'));
			$this->packagedetails->setDbValue($rs->fields('packagedetails'));
			...
			$this->pdate->setDbValue($rs->fields('pdate'));
			$this->Val[1] = $this->peroid->CurrentValue;
			$this->Val[2] = $this->packagename->CurrentValue;
			$this->Val[3] = $this->price->CurrentValue;
			$this->Val[4] = $this->packagedetails->CurrentValue;
			$this->Val[5] = $this->fname->CurrentValue;
			$this->Val[6] = $this->lname->CurrentValue;
			$this->Val[7] = $this->dob->CurrentValue;
			$this->Val[8] = $this->gender->CurrentValue;
			$this->Val[9] = $this->weight->CurrentValue;
			$this->Val[10] = $this->phn->CurrentValue;
			$this->Val[11] = $this->pdate->CurrentValue;
		} else {
			$this->peroid->setDbValue("");
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			// peroid
			$this->peroid->ViewValue = $this->peroid->CurrentValue;
			$this->peroid->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";

			// packagename
			$this->packagename->ViewValue = $this->packagename->CurrentValue;
			$this->packagename->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
			...
			$this->packagedetails->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
			...
			// pdate
			$this->pdate->ViewValue = $this->pdate->CurrentValue;
			$this->pdate->ViewValue = ewr_FormatDateTime($this->pdate->ViewValue, 0);
			$this->pdate->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// peroid
			$CurrentValue = $this->peroid->CurrentValue;
			$ViewValue = &$this->peroid->ViewValue;
			$ViewAttrs = &$this->peroid->ViewAttrs;
			$CellAttrs = &$this->peroid->CellAttrs;
			$HrefValue = &$this->peroid->HrefValue;
			$LinkAttrs = &$this->peroid->LinkAttrs;
			$this->Cell_Rendered($this->peroid, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
			...
			// pdate
			$CurrentValue = $this->pdate->CurrentValue;
			$ViewValue = &$this->pdate->ViewValue;
			$ViewAttrs = &$this->pdate->ViewAttrs;
			$CellAttrs = &$this->pdate->CellAttrs;
			$HrefValue = &$this->pdate->HrefValue;
			$LinkAttrs = &$this->pdate->LinkAttrs;
			$this->Cell_Rendered($this->pdate, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
		}
	...
	function SetupFieldCount() {
		...
		if ($this->peroid->Visible) $this->DtlColumnCount += 1;
		if ($this->packagename->Visible) $this->DtlColumnCount += 1;
		if ($this->price->Visible) $this->DtlColumnCount += 1;
		if ($this->packagedetails->Visible) $this->DtlColumnCount += 1;
		...
		if ($this->pdate->Visible) $this->DtlColumnCount += 1;
	}
...
// Create page object
if (!isset($package_report_rpt)) $package_report_rpt = new crpackage_report_rpt();
...
// Create page object
var package_report_rpt = new ewr_Page("package_report_rpt");

// Page properties
package_report_rpt.PageID = "rpt"; // Page ID
var EWR_PAGE_ID = package_report_rpt.PageID;
</script>
<?php } ?>
...
<!-- Report grid (begin) -->
<?php if ($Page->Export <> "pdf") { ?>
<div id="gmp_package_report" class="<?php if (ewr_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php } ?>
<table class="<?php echo $Page->ReportTableClass ?>">
<thead>
	<!-- Table header -->
	<tr class="ewTableHeader">
<?php if ($Page->peroid->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="peroid"><div class="package_report_peroid"><span class="ewTableHeaderCaption"><?php echo $Page->peroid->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="peroid">
<?php if ($Page->SortUrl($Page->peroid) == "") { ?>
		<div class="ewTableHeaderBtn package_report_peroid">
			<span class="ewTableHeaderCaption"><?php echo $Page->peroid->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer package_report_peroid" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->peroid) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->peroid->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->peroid->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->peroid->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="packagename"><div class="package_report_packagename"><span class="ewTableHeaderCaption"><?php echo $Page->packagename->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="packagename">
<?php if ($Page->SortUrl($Page->packagename) == "") { ?>
		<div class="ewTableHeaderBtn package_report_packagename">
			<span class="ewTableHeaderCaption"><?php echo $Page->packagename->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer package_report_packagename" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->packagename) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->packagename->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->packagename->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->packagename->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="price"><div class="package_report_price"><span class="ewTableHeaderCaption"><?php echo $Page->price->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="price">
<?php if ($Page->SortUrl($Page->price) == "") { ?>
		<div class="ewTableHeaderBtn package_report_price">
			<span class="ewTableHeaderCaption"><?php echo $Page->price->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer package_report_price" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->price) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->price->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="packagedetails"><div class="package_report_packagedetails"><span class="ewTableHeaderCaption"><?php echo $Page->packagedetails->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="packagedetails">
<?php if ($Page->SortUrl($Page->packagedetails) == "") { ?>
		<div class="ewTableHeaderBtn package_report_packagedetails">
			<span class="ewTableHeaderCaption"><?php echo $Page->packagedetails->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer package_report_packagedetails" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->packagedetails) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->packagedetails->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->packagedetails->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->packagedetails->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="fname"><div class="package_report_fname"><span class="ewTableHeaderCaption"><?php echo $Page->fname->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="fname">
<?php if ($Page->SortUrl($Page->fname) == "") { ?>
		<div class="ewTableHeaderBtn package_report_fname">
			<span class="ewTableHeaderCaption"><?php echo $Page->fname->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer package_report_fname" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->fname) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->fname->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="lname"><div class="package_report_lname"><span class="ewTableHeaderCaption"><?php echo $Page->lname->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="lname">
<?php if ($Page->SortUrl($Page->lname) == "") { ?>
		<div class="ewTableHeaderBtn package_report_lname">
			<span class="ewTableHeaderCaption"><?php echo $Page->lname->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer package_report_lname" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->lname) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->lname->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="dob"><div class="package_report_dob"><span class="ewTableHeaderCaption"><?php echo $Page->dob->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="dob">
<?php if ($Page->SortUrl($Page->dob) == "") { ?>
		<div class="ewTableHeaderBtn package_report_dob">
			<span class="ewTableHeaderCaption"><?php echo $Page->dob->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer package_report_dob" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->dob) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->dob->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="gender"><div class="package_report_gender"><span class="ewTableHeaderCaption"><?php echo $Page->gender->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="gender">
<?php if ($Page->SortUrl($Page->gender) == "") { ?>
		<div class="ewTableHeaderBtn package_report_gender">
			<span class="ewTableHeaderCaption"><?php echo $Page->gender->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer package_report_gender" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->gender) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->gender->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="weight"><div class="package_report_weight"><span class="ewTableHeaderCaption"><?php echo $Page->weight->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="weight">
<?php if ($Page->SortUrl($Page->weight) == "") { ?>
		<div class="ewTableHeaderBtn package_report_weight">
			<span class="ewTableHeaderCaption"><?php echo $Page->weight->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer package_report_weight" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->weight) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->weight->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="phn"><div class="package_report_phn"><span class="ewTableHeaderCaption"><?php echo $Page->phn->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="phn">
<?php if ($Page->SortUrl($Page->phn) == "") { ?>
		<div class="ewTableHeaderBtn package_report_phn">
			<span class="ewTableHeaderCaption"><?php echo $Page->phn->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer package_report_phn" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->phn) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->phn->FldCaption() ?></span>
...
<?php } ?>
<?php } ?>
<?php if ($Page->pdate->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="pdate"><div class="package_report_pdate"><span class="ewTableHeaderCaption"><?php echo $Page->pdate->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="pdate">
<?php if ($Page->SortUrl($Page->pdate) == "") { ?>
		<div class="ewTableHeaderBtn package_report_pdate">
			<span class="ewTableHeaderCaption"><?php echo $Page->pdate->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer package_report_pdate" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->pdate) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->pdate->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->pdate->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->pdate->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<?php if ($Page->peroid->Visible) { ?>
		<td data-field="peroid"<?php echo $Page->peroid->CellAttributes() ?>>
<span<?php echo $Page->peroid->ViewAttributes() ?>><?php echo $Page->peroid->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->packagename->Visible) { ?>
		<td data-field="packagename"<?php echo $Page->packagename->CellAttributes() ?>>
<span<?php echo $Page->packagename->ViewAttributes() ?>><?php echo $Page->packagename->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->price->Visible) { ?>
...
<span<?php echo $Page->packagedetails->ViewAttributes() ?>><?php echo $Page->packagedetails->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->fname->Visible) { ?>
...
<?php } ?>
<?php if ($Page->pdate->Visible) { ?>
		<td data-field="pdate"<?php echo $Page->pdate->CellAttributes() ?>>
<span<?php echo $Page->pdate->ViewAttributes() ?>><?php echo $Page->pdate->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
...
<?php include "package_reportrptpager.php" ?>
